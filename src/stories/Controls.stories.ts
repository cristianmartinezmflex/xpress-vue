...
export const SelectDynamic = story(
  { id: 'site_id', type: 'select_dynamic', title: 'Site', loadFrom: 'sites', default: -1 },
  {
    ...
    docs: [
      ...
      '- `"get-<type>"` → `POST /dm/{guid}/custom` con body `{ action: "get-<type>" }` (data específica del DM, ej. `get-sites`, `get-directories`).',
...
export const MultiselectDynamic = story(
  { id: 'panel_filter', type: 'multiselect', title: 'Panels', loadFrom: 'panels' },
  {
    ...
    docs: [
      ...
      '',
      '**Props soportadas:**',
      '- `loadFrom` — fuente para **auto-cargar** la lista al montar (misma convención que `select_dynamic`: `"get-<type>"` → `POST /dm/{guid}/custom`, `"shared/<type>"` → `GET /api/shared/`). Muestra un spinner mientras carga.',
